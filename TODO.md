...
- [ ] **상대 모델링(StatsService 확장) + 유형별 착취** — 해링턴 Part 2 수치 사전
      (F3B/AF/WtSD/W$SD 등) 집계 후 니트/콜스테이션/LAG 판별 → PostflopAdvisor 의
      경보·블러프·밸류 기준을 상대 유형으로 보정
      배치: bot·stats  |  완료기준: 상대 유형별 판단 분기 + 테스트  |  참고: PROMPTS.md
...
> 현재 전체 130개 단위·통합 테스트 통과(차트 스모크는 로컬 전용 — 파일 없으면 자동 스킵).
